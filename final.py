...
def run_cross_validation(clf_list, featureName, featureMatrix, labelMatrix, event_num=EVENT_NUM, seed=0, num_folds=5, num_epochs=100, batch_size=128
    ...
    for clf in clf_list:
      ...
      # Save the final model if it's CNN_DDI
      if clf == 'CNN_DDI':
          ...
          x_train = all_matrix[0]  # Using the first feature matrix
          ...
          final_model = CNN_DDI(loss_fn=loss_fn)
          final_model.fit(x_train_reshaped, y_train_one_hot, 
                         batch_size=batch_size, 
                         epochs=num_epochs,
                         verbose=1)
          
          # Save the model
          model_path = 'models/cnn_ddi_model.h5'
